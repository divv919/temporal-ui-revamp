          ...
          {preFooter.buttons.map((btn, index) => {
            return (
              <Link href={btn.href} key={btn.name + index}>
                <button className="text-md bg-white/90 flex gap-1 items-center justify-center text-green-800 text-sm  font-medium px-2 shadow-[0px_10px_8px_rgba(0,0,0,0.08)]  py-2 rounded-xl relative">
                  {index === 0 ? (
                    <Book size={16} />
                  ) : index === 1 ? (
                    <Code size={16} />
