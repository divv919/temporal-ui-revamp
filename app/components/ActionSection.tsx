...
export default function ActionSection() {
  return (
    <motion.div
      ...
      className="w-full h-60 lg:h-100 px-7 lg:px-25  mt-15 lg:mt-40"
    >
      <div className="w-full h-full shadow-green-500/10 shadow-xl tracking-tight bg-green-700/30 flex flex-col gap-0 justify-center items-center relative rounded-2xl overflow-hidden">
        ...
        ></motion.div>
        <div className="text-2xl lg:text-5xl text-green-50  relative">
          {preFooter.title}
        </div>
        <div className="text-sm lg:text-lg text-green-200/90 relative mt-1">
          {preFooter.subtitle}
        </div>
          ...
          {preFooter.buttons.map((btn, index) => {
            return (
              <Link href={btn.href} key={btn.name + index}>
                <button className="cursor-pointer hover:bg-white/80 transition-colors duration-200 text-[10px] lg:text-[14px] bg-white/90 flex gap-1 items-center justify-center text-green-800 text-sm  font-medium px-1.5 lg:px-2 shadow-[0px_10px_8px_rgba(0,0,0,0.08)] py-1.5  lg:py-2 rounded-xl relative">
                  {index === 0 ? (
